...
let user = message.mentions.users.first();
let muteRole = message.guild.roles.find("name", "Muted");
if (!muteRole) return message.reply("** لا يوجد ر��بة الميوت 'Muted' **").then(msg => {msg.delete(5000)});
if (message.mentions.users.size < 1) return message.reply('** يجب عليك المنشن اولاً **').then(msg => {msg.delete(5000)});
let reason = message.content.split(" ").slice(2).join(" ");
...
if (command == "warn") {
    ...
    client.channels.get("466409418662215680").send(`**=========================================**`)
    client.channels.get("466409418662215680").send(`**New Warn !**`)
    client.channels.get("466409418662215680").send({embed : say})
    client.channels.get("466409418662215680").send(`**Admin : ${message.author.username}#${message.author.discriminator}**`)
    client.channels.get("466409418662215680").send(`**In Channel : ${message.channel}**`)
    message.delete();
  }
...
client.on('message', message => {                      
    if(!message.channel.guild) return;
       ...
       message.channel.sendMessage(`اض��ط على الصح عشان تتفعل`).then(msg => {
...
let y5Filter = (reaction, user) => reaction.emoji.name === '💙' && user.id === message.author.id;
     ...
     const embed = new Discord.RichEmbed() 
      ...
      .setDescription("**:art:��م اعطائك اللون اللبني**")
      .setFooter(message.author.tag , message.author.avatarURL)
...
client.on("guildMemberAdd", (member) => {
    ...
    let channel = member.guild.channels.get("451762950391201794");
